...
`DATAJUD_API_KEY` must be provided via environment/secret manager in production.
...
### Verification policy (startup safety vs brittleness)

The watcher supports two verification modes at startup:

- Non-strict mode (default): verify best effort and continue polling when
  optional capabilities are missing.
- Strict mode: fail fast if verification checks are incomplete.

Checks performed:

1. TPU movement mapping check for `51`, `901`, `981` against CNJ public tables.
2. Probe response check using a known case (`--probe-case` + `--probe-tribunal`):
   - Core required fields: `_source.numeroProcesso`, `_source.movimentos`
   - Optional capability fields: `_source.processoRelacionado`,
     `_source.partes[].documento`

Rationale for non-strict default: real DataJud payloads vary by tribunal and
dataset. Some valid responses omit `partes` or `processoRelacionado`, so hard
failing on those fields is brittle and can stop the watcher without reducing
data risk.

CLI flags:

```txt
--verify-tpu=true|false
--strict-verify=true|false
--probe-case=<numeroProcesso>
--probe-tribunal=<api_publica_trfX|api_publica_stj|...>
```

Operational note: `api_publica_stf` has returned `404` in live probing with the
currently provided key. Prefer TRF/STJ probe tribunals unless STF access is
confirmed.
